...
const TravelCard = ({ travel, groupid, onClick, className }) => {
  ...
  return (
    <div
      className={`bg-white backdrop-blur-sm rounded-3xl p-4 my-4 mx-1 shadow-[0_0_5px_rgba(0,0,0,0.1)] transition-all duration-200 cursor-pointer transform hover:-translate-y-1 flex flex-row items-center space-x-4 ${className}`}
      onClick={onClick}
      ...
      <div className="flex items-center space-x-1">
        ...
        <div className="flex flex-col space-y-1">
          <div className="flex space-x-1 items-end">
            ...
            <p className="text-xs text-gray-500 mb-1 max-w-24 line-clamp-1 truncate">
              {' '}
              | {travel.location}
            </p>
          </div>
