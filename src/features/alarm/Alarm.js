...
const NotificationItem = React.memo(({ notification }) => {
  ...
  return (
    <div
      ...
      tabIndex={0}
      className={`w-full flex items-center p-3 hover:bg-gray-6 transition-colors shadow-[0_0_10px_rgba(0,0,0,0.1)] rounded-xl
        ${notification.isAccepted ? 'bg-gray-6' : 'bg-white'}`}
    >
      {/* 사진 */}
      ...
      <div className="w-full flex flex-row items-center justify-between gap-2">
        <div className="ml-4 flex-1 min-w-0">
          {notification.groupName && (
            <span className="text-medium font-semibold text-gray-900 flex-shrink-0 truncate flex-1">
              {notification.groupName}
            </span>
          )}
