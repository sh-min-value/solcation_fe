...
const CardTransactionHistory = ({
  ...
  useEffect(() => {
    const fetchTransactions = async () => {
      ...
      setLoading(true);
      try {
        const response = await TransactionAPI.getTransactionsCard(
          groupId,
          yearMonth
        );
        ...
        console.error('카드 거래내역 조회 오류:', error);
        navigate('/error', {
          state: {
  ...
  const getSelectedMonthLabel = () => {
    const option = monthOptions?.find(opt => opt.value === yearMonth);
  ...
  return (
    <div className="w-full px-4 py-4">
      ...
      <div className="w-full mt-1 mb-4 relative">
        <div
          className="inline-flex items-center cursor-pointer px-2 pl-3 py-1 w-[120px] flex flex-row justify-between"
        ...
        >
          <span className="text-sm text-gray-2">{getSelectedMonthLabel()}</span>
          <svg
        ...
        {showMonthDropdown && (
          <div className="absolute z-10 bg-white border border-gray-5 rounded-md shadow-lg mt-1 min-w-fit w-16">
            {monthOptions?.map(option => (
              <div
                key={option.value}
                className={`px-2 pl-3 py-2 text-sm cursor-pointer text-left w-[120px] ${
                ...
                role="option"
                tabIndex={0}
                aria-selected={yearMonth === option.value}
                onClick={() => {
...
CardTransactionHistory.propTypes = {
