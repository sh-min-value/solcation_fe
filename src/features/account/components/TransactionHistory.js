  ...
  return (
    <div className="w-full px-4 py-4">
      <div className="mt-6 mb-4 relative">
        <div
          className="inline-flex items-center cursor-pointer px-2 py-1"
          ...
          <div className="absolute z-10 bg-white border border-gray-200 rounded-md shadow-lg mt-1 min-w-fit">
            {filterOptions.map(option => (
              <div
                ...
                className={`px-3 py-2 text-sm cursor-pointer text-center ${
                  filter === option.value
                    ? 'bg-main text-white'
      ...
      {Object.keys(groupedTransactions).length === 0 ? (
        ...
        Object.entries(groupedTransactions).map(([date, dayTransactions]) => (
          <div key={date} className="space-y-4 mb-6">
            {/* 날짜 헤더 */}
            <div className="text-gray-2 text-sm font-medium mb-2">{date}</div>
                ...
                >
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <div className="bg-light-blue rounded px-2 py-0.5 flex items-center text-[8px]">
                        {getTransactionCategoryIcon(
                          transaction.tcCode,
                          'w-2.5 h-2.5 text-gray-2 mr-1'
                        )}
                      </div>
                    </div>
